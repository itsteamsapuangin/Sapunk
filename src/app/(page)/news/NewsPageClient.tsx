            ...
            <div className="flex flex-col lg:flex-row lg:items-end gap-6 lg:gap-8">
                ...
                <div className="flex flex-col gap-5 lg:w-85 xl:w-100 shrink-0 order-2 lg:order-first lg:mt-10">
                    ...
                    <div className="flex flex-col gap-4">
                        <h2 className="text-[34px] font-extrabold leading-[1.25] tracking-tight text-black">
                            {article.title}
                        </h2>
        ...
        <div className="h-full overflow-hidden rounded-xl border border-[#ccc] bg-white p-5 shadow-[0_0_12px_rgba(0,0,0,0.05)]">
            <div className="flex h-full flex-col gap-6">
                ...
                <div className="flex flex-1 flex-col gap-3">
                    <CategoryBadge label={article.categoryLabel} color={article.categoryColor} />
                    <h3 className="text-2xl font-extrabold leading-[1.25] tracking-tight text-black">
                        {article.title}
                    </h3>
        ...
        <div className="h-full overflow-hidden rounded-xl border border-[#ccc] bg-white p-5 shadow-[0_0_12px_rgba(0,0,0,0.05)]">
            <div className="flex flex-col md:flex-row items-end gap-5 h-full">
                ...
                <div className="flex flex-col gap-5 justify-end md:w-[45%] lg:w-87.75 shrink-0 order-last md:order-0">
                    <CategoryBadge label={article.categoryLabel} color={article.categoryColor} />
                    <h3 className="text-2xl lg:text-[34px] font-extrabold leading-[1.25] tracking-tight text-black">
                        {article.title}
                    </h3>
                    <p className="text-base font-medium leading-normal tracking-tight text-[#5d5d5d]">
                        {article.excerpt}
        ...
        <div className="h-full overflow-hidden rounded-xl border border-[#ccc] bg-white p-5 shadow-[0_0_12px_rgba(0,0,0,0.05)]">
            <div className="flex flex-col md:flex-row items-stretch gap-5 h-full">
                ...
                <div className="flex flex-1 flex-col gap-5 justify-end md:items-end md:text-right">
                    <CategoryBadge label={article.categoryLabel} color={article.categoryColor} />
                    <h3 className="text-2xl lg:text-[34px] font-extrabold leading-[1.25] tracking-tight text-black w-full">
                        {article.title}
                    </h3>
...
export default function NewsPageClient() {
    ...
    const activeCategory =
        FILTER_CATEGORIES.find((c) => c.id === activeFilter) ?? FILTER_CATEGORIES[0];
    ...
    return (
        <div className="flex flex-col items-start bg-white relative w-full">
            {/* Visually hidden h1 for SEO — the visual title is the "Sapang Times" SVG */}
            <h1 className="sr-only">Sapang Times — ITS Team Sapuangin News</h1>
            ...
            <section className="w-full bg-white px-6 md:px-12 lg:px-24 py-8 md:py-16">
                <div className="mx-auto max-w-312 flex flex-col gap-8">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        {/* Filter dropdown */}
                        <div ref={dropdownRef} className="relative self-end shrink-0">
                            <button
                                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                                className={`flex items-center gap-2 rounded-lg border border-[#e0e0e0] px-4 py-2.5 text-sm font-semibold tracking-tight text-[#555] bg-[#fafafa] hover:bg-[#f0f0f0] transition-colors cursor-pointer`}
                            >
                                <span>Filter: {activeCategory.label}</span>
                                <svg
                            ...
                            {/* Dropdown menu */}
                            <div
                                className={`absolute right-0 top-full mt-2 z-50 min-w-52 overflow-hidden rounded-lg border border-[#ccc] bg-white shadow-lg transition-all duration-200 ${
                                    isDropdownOpen
                                ...
                                {FILTER_CATEGORIES.map((cat) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-semibold tracking-tight transition-colors cursor-pointer ${
                                                isActive
                                                    ? 'bg-[#f5f5f5] text-[#333]'
